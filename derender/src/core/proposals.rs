...
dyngen!(
pub fn rgbd_drift(trace: Weak<DynTrace<(f32,f32,bool),(Depths,Colors)>>, mask: Vec<&str>, stdev: f32) {
...
pub fn pose_guide(_trace: Weak<DynTrace<(f32,f32,bool),(Depths,Colors)>>, targets: Vec<(&str, f32, f32)>) {
...
dyngen!(
pub fn noise_drift(trace: Weak<DynTrace<f32,Colors>>, mask: Vec<&str>, stdev: f32) {
    let trace = trace.upgrade().unwrap();
    for addr in mask.iter() {
        normal(trace.data.read::<f32>(addr), stdev) %= addr;
    }
