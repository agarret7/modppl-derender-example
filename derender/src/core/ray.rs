...
use crate::config::{FAR, H, NEAR, VP, W};
use crate::core::solid::{Solid,Scene};
use crate::image::*;
...
pub fn raytrace_depths(x: Affine3A, proj: Mat4, scene: &Vec<Box<dyn Solid>>, out: &mut Depths) {
    ...
    out.par_chunks_mut(W()).enumerate().for_each(|(y, row)| {
        for x in 0..W() {
            ...
            for s in scene.iter() {
                if let Some(d) = s.ray_intersect(ray_origin, ray_dir) {
...
pub fn raytrace_depths_and_flat_colors(
    ...
    depths_out: &mut Depths,
    colors_out: &mut Colors,
) {
    let i = x * proj.inverse();
    // rays start at the camera center, not on the near plane: a near-plane
    // origin makes geometry closer than NEAR structurally invisible (origins
