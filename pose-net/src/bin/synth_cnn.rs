...
fn main() -> anyhow::Result<()> {
    ...
    run_sandbox_loop(
        ...
        || {
            ...
            let est = estimator
                .estimate(&obs_depth, &obs_color)
                .expect("CNN forward failed");
            let az_err = {
            ...
            let kernel = InferenceKernel::new(&cube_rgbd_model)
                .regen_mh(&cube_pass)
                    ...
                    if cnn_enabled.get() && est.azimuth_confidence > MIN_AZIMUTH_CONFIDENCE {
                        ...
                        let (t, accepted) = mh(
                            ...
                            vec![
                                ...
                                (
                                    "orbit_sin_elevation",
                                    est.sin_elevation.clamp(0.05, 0.95),
                                    0.1,
                                ),
                        ...
                        if accepted {
                            cnn_accepts.set(cnn_accepts.get() + 1);
                        }
            ...
            move |keys: &[Key]| {
                if keys.contains(&Key::C) {
                    ...
                    println!(
                        "cnn moves: {}",
                        if cnn_enabled.get() { "ON" } else { "OFF" }
                    );
                }
